...
import { Mail, Lock, User, ArrowRight, Sparkles, Shield, Zap, FileText } from "lucide-react";
...
export default function AuthPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-hero p-4">
      <SEOHead
        title="Sign In or Create Free Account — PineToolsHub AI Productivity OS"
        description="Sign in or create a free PineToolsHub account to save files to cloud, access AI features, create automations and sync across devices. All 30+ tools work free without login."
        canonical="/auth"
        noindex
      />
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="flex items-center justify-center gap-3 mb-8">
          <img src={logo} alt="PineToolsHub Logo" className="h-12 w-12 rounded-xl" />
          <div>
          ...
          <CardContent>
            ...
            <div className="mt-6 grid grid-cols-3 gap-2">
              {[
                ...
                { icon: Zap, label: "30+ Tools" },
                { icon: FileText, label: "No Limits" },
              ].map((item) => (
                <div key={item.label} className="flex flex-col items-center gap-1 p-2 rounded-lg bg-sidebar-accent/50">
                  <item.icon className="h-4 w-4 text-pink-400" />
                  <span className="text-[10px] text-sidebar-foreground/60 font-medium">{item.label}</span>
                </div>
              ))}
            </div>
